...
国が管理する地名は、自然地形、自治体（州・市郡・区邑郷）、空港、港、片（区邑郷の下位・後述）、住所（坊通町村・番巷字・号）、鉄道と軌道（会社・路線・駅・站・側線・操車場・信号場・車両基地など）、道路（路線・IC・JCT・料金所・交差点・道の駅・自動車駅・電停・バス停など）、公共建物の名前と番号である。本レポジトリの「List_objects.yaml」にはこれらの地名の一覧（制作中）が公開されている。スイス連邦の地名条例（Verordnung über die geografischen Namen）と時刻表条例（Fahrplanverordnung）は伊良島でも有効。鉄道も道路も左側通行。
